...
const ProjectCard = ({ project }) => {
  ...
  return (
    <>
      <div className="w-full h-[350px] bg-slate-900 p-6 border border-slate-700 hover:border-slate-500 transition-colors">
        <BlurImage
          src={image}
          alt={title}
          className="w-full h-full object-cover"
          containerClassName="w-full h-40 border border-slate-700"
        />
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={() => setModalOpen(false)}>
          <div className="bg-slate-900 border border-slate-700 relative w-full max-w-4xl max-h-[90vh] overflow-auto modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <BlurImage
              ...
              alt={title}
              className="w-full h-full object-cover"
              containerClassName="h-48 sm:h-56 md:h-64 border-b border-slate-700"
            />
